...
const YoutubeIcon = () => (
  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 0 0-1.95 1.96A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z" /><polygon fill="white" points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" /></svg>
);
const InstagramIcon = () => (
...
const ICON_MAP: Record<string, { Icon: React.FC; bg: string }> = {
  social_facebook:  { Icon: FacebookIcon,  bg: "hover:bg-[#1877f2]" },
  social_x:         { Icon: XIcon,          bg: "hover:bg-black" },
  social_youtube:   { Icon: YoutubeIcon,    bg: "hover:bg-[#ff0000]" },
  social_instagram: { Icon: InstagramIcon,  bg: "hover:bg-pink-600" },
  social_linkedin:  { Icon: LinkedInIcon,   bg: "hover:bg-[#0077b5]" },
  social_whatsapp:  { Icon: WhatsAppIcon,   bg: "hover:bg-[#25d366]" },
};

// Fallback
const FALLBACK = {
  footer_slogan: "Where positivity begins",
  footer_about: "Gülüşünüz bizim için değerli. İstanbul Nişantaşı ve Adana Türkmenbaşı şubelerimizde modern teknoloji ve uzman kadroyla yanınızdayız.",
  footer_copyright: "© 2026 Positive Dental Studio. Tüm hakları saklıdır.",
  footer_phone: "0850 123 45 67",
  footer_email: "info@positivedental.com",
  footer_hours: "Pzt – Cmt: 09:00 – 20:00",
  footer_hours_note: "Cumartesi: 09:00 – 18:00",
  footer_links: JSON.stringify([
    { to: "/", label: "Ana Sayfa" }, { to: "/hizmetlerimiz", label: "Hizmetlerimiz" },
    { to: "/fiyat-listesi", label: "Fiyat Listesi" }, { to: "/hakkimizda", label: "Hakkımızda" },
    { to: "/kliniklerimiz", label: "Kliniklerimiz" }, { to: "/blog", label: "Blog" }, { to: "/iletisim", label: "İletişim" },
  ]),
  footer_services: JSON.stringify(["Genel Diş Hekimliği", "İmplant Tedavisi", "Estetik Diş Hekimliği", "Ortodonti", "Çocuk Diş Hekimliği"]),
  social_facebook: "#", social_x: "#", social_youtube: "#", social_instagram: "#", social_linkedin: "#", social_whatsapp: "https://wa.me/905001234567",
};

export function Footer() {
  const [vals, setVals] = useState<Record<string, string>>(FALLBACK);

  useEffect(() => {
    supabase.from("site_settings").select("*").in("group_name", ["footer", "footer_social"]).then(({ data }) => {
    ...
    <footer className="bg-[#0D1235] text-blue-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-blue-100">{vals.footer_phone}</p>
                  <p className="text-xs text-blue-400">Randevu Hattı</p>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <Mail className="w-4 h-4 mt-0.5 flex-shrink-0 text-blue-400" />
                <p className="text-sm text-blue-100">{vals.footer_email}</p>
              </li>
              <li className="flex items-start gap-3">
                <Clock className="w-4 h-4 mt-0.5 flex-shrink-0 text-blue-400" />
                <div>
                  <p className="text-sm text-blue-100">{vals.footer_hours}</p>
                  <p className="text-xs text-blue-400">{vals.footer_hours_note}</p>
                </div>
              </li>
            </ul>
          </div>
        ...
        <div className="border-t border-white/10 mt-10 pt-8 flex flex-col sm:flex-row justify-between items-center gap-3">
          ...
          <a href="/online-randevu" title="Online randevu al"
